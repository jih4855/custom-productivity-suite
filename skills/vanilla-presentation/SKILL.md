...
### 0.4 슬라이드 수와 배치는 사전 협의
- 본 작성 전 **사용자와 슬라이드 매수·배치(2D 그리드 좌표)·구성 컴포넌트를 협의**한다.
...
- 사전 협의 항목 체크리스트:
  ...
  - [ ] 2D 좌표 배치(예: 3x2)
  - [ ] 사용할 테마 — 이용자에게 `templates/` 카탈로그를 제시해 선택받는다
...
### 0.6 테마 선택과 레퍼런스 기반 신규 테마
- **테마는 이용자가 선택한다.** 사전 협의에서 `templates/` 폴더의 등재된 테마 목록과 각 톤 특징을 제시한 뒤 하나를 고르게 한다.
- 선택된 테마는 빌드 시 결과물 폴더의 `theme.css`로 복사된다. 공통 프레임은 `base.css`, 내비게이션 엔진은 `engine.js`로 함께 복사된다.
- **이용자가 외부 레퍼런스(사이트 URL·이미지·무드보드 등)를 제공하면**, 그 레퍼런스를 주제 삼아 새 테마 CSS를 만든다.
  - 색상 팔레트·타이포 사이즈·간격을 레퍼런스에서 추출해 변수(`--blue` / `--navy` / `--block-gap` 등)에 반영한다.
  - 컴포넌트(card / node / map / table / banner)의 골격은 기존 테마 중 가장 비슷한 것을 베이스로 한다.
- 결과물에서 신규 테마가 검증되면 `templates/<주제명>.css`로 회수해 카탈로그에 등재한다.
...
- `templates/<테마>.css`: 테마 카탈로그. 빌드 시 `theme.css`로 복사됨. 새 테마는 여기에 추가해 재사용한다.
...
### 5.1 작업 순서
1.  **사전 협의 및 승인 (Mandatory)**: 본 작업 전 반드시 사용자에게 디자인 방향성(테마)과 슬라이드 구성(개수, 컴포넌트 배치)을 제안하고 **명시적인 승인**을 받는다. 임의로 설계를 진행하지 않는다.
2.  **JSON 설계**: 승인된 구조에 따라 [§3. JSON 스펙 가이드](#3-json-스펙-가이드)를 참고하여 `spec.json`을 먼저 생성한다.
3.  **테마 CSS 구현 및 예제 검증**: [§4. 테마 CSS 작성 가이드](#4-테마-css-작성-가이드)에 따라 모든 컴포넌트를 구현하고, **`assets/examples/`에 갤러리를 생성하여 전수 검증 및 사용자 피드백을 받는다.**
4.  **빌드 실행**: `build.py`를 실행하여 `index.html`, `base.css`, `theme.css`, `engine.js`를 생성한다.
...
2.  **검증 및 원본 반영**: 사용자가 확정한 수정 사항에 한해, 해당 내용을 `assets/templates/<테마>.css` 원본 파일에 역으로 반영한다.
3.  **일관성 유지**: 이를 통해 특정 프로젝트에서의 개선사항이 전체 테마 시스템의 자산으로 축적되도록 한다.

### 5.3 사용자 경험 (UX) 강화
- **수정 유도**: "스타일을 직접 수정하려면 결과물 폴더의 `theme.css`를 수정하면 됩니다." 라고 안내한다.
- **내비게이션 안내**: 좌우 화살표 이동과 사이드바 네비게이션 사용법을 반드시 언급한다.
