...
│   ├── templates/
│   │   └── minimal-tech-hero.css # 맞춤형 theme.css 제작을 위한 단일 레퍼런스
│   └── examples/
...
1.  **사전 협의**: 작업을 시작하기 전, 반드시 사용자에게 **디자인 방향(요청·레퍼런스·산업 톤)**과 **슬라이드 구성**을 제안하고 승인을 받습니다.
2.  **맞춤형 테마 작성**: `minimal-tech-hero.css`를 구현 예제로 참고하되, 실제 발표 자료는 결과물 폴더의 `theme.css`를 사용자 맞춤형으로 작성합니다.
3.  **Bottom-up 튜닝**: 결과물이 나온 뒤 세부 디자인을 수정할 때는 생성된 `theme.css`를 먼저 고쳐서 확정받고, 반복 사용 가치가 있을 때만 템플릿으로 회수합니다.
...
- **자유로운 수정**: `minimal-tech-hero`는 완성 테마라기보다 모든 컴포넌트 스타일 구현을 보여주는 예제입니다. 실제 발표 자료는 사용자 요청과 레퍼런스에 맞춰 결과물 폴더의 `theme.css`를 새로 작성하고, 검증된 스타일만 재사용 템플릿으로 회수합니다.
